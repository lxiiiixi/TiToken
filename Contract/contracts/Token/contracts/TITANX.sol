...
/** @title Titan X */
contract TITANX is ITITANX, ERC20, ReentrancyGuard, GlobalInfo, MintInfo, StakeInfo, BurnInfo, Invitation, OwnerInfo {
    /** Storage Variables*/
  ...
  function claimAllYieldAndGas() external {
    ...
    function startStake(uint256 amount, uint256 numOfDays) external dailyUpdate nonReentrant {
        if (balanceOf(msg.sender) < amount) revert TitanXErrors.TitanX_InsufficientBalance();
